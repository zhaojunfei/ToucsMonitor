 ...
 * @version 1.0
 */
public class POSICCardServlet extends CommonServlet {
	/**
	 * 
	 */
	...
	private static final String MODCARDREQ1="1300201";  //从查询页面发起的修改商业IC卡卡表信息请求码
	private static final String MODCARDREQ2="1300202";  //从管理页面发起的修改商业IC卡卡表信息请求码
	private static final String DELCARD="13003";  //删除商业IC卡卡表信息请求码
	private static final String QUERYCARD="13004";  //查询商业IC卡卡表信息请求码
	private static final String QUERYHANDCARD="13005";	//商业IC卡手输卡号查询
	private static final String CANCEL="CANCEL";  //取消请求码
	private static final String SETHANDFLAG="13006";//设置商业IC卡手输标志交易
	...
	//业务Bean的实例
	private static POSICCardBean parameterSetBean=new POSICCardBean();
	...
	private String execDoGet(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException,MonitorException{
		...
		try{
			if(reqCode.equals(REGCARD)){
				...
				lTarger="/ToucsMonitor/POSDeviceManage/CardReg.jsp";
			}else if(reqCode.equals(MODCARD)){
				QueryCode queryCode=new QueryCode();
				queryCode.query(1);
				request.setAttribute("bankCode",queryCode.getBankCode()) ;
				request.setAttribute("reqCode",MODCARDREQ1);
				lTarger="/ToucsMonitor/POSDeviceManage/CardQuery.jsp";
				...
				lTarger="/ToucsMonitor/POSDeviceManage/HandCardQuery.jsp";
				...
				if (hand_flag == null || Track_no == null ||
					...
					throw new MonitorException(ErrorDefine.GETTXCODEERR,
							ErrorDefine.GETTXCODEERRDESC);
				}
				...
				if (hand_flag.trim().equalsIgnoreCase("1") ) {
					...
					if (hand_flag.trim().equalsIgnoreCase("0") ) {
				...
				String url="/ToucsMonitor/posicservlet?reqCode=13005&page="+showPage+"&isFirstTime=0"+"&reQueryFlag=1";
		...
		try{
			if(reqCode.equals(REGCARD)){
				...
				try{
					...
					String target="/ToucsMonitor/posicservlet?reqCode=13004";
					String btnTitle="查询商业IC卡卡表";
					request.setAttribute("targetURL",target);
					request.setAttribute("btnTitle",btnTitle);
					Message="成功增加商业IC卡卡表信息到数据中，但";
					throw new MonitorException(mexp.getErrCode(),Message+mexp.getMessage());
				}finally{
					//设置日志信息
					String userOperRstDesc="新增加一条商业IC卡卡表信息,卡表信息:";
				...
				Message="增加商业IC卡卡表信息成功.";
				request.setAttribute("Message",Message);
				PromptBean prompt=new PromptBean("商业IC卡卡表信息管理");
				prompt.setPrompt(Message);
				prompt.setButtonUrl(0,"添加","/ToucsMonitor/posicservlet?reqCode="+REGCARD);
				prompt.setButtonUrl(1,"查询","/ToucsMonitor/posicservlet?reqCode="+QUERYCARD);
				request.setAttribute("prompt",prompt);
				lTarget="/ToucsMonitor/Success.jsp";
				//写入日志
			}else if(reqCode.equals(MODCARD)){
				//接收到修改卡表信息请求，取得修改的值，提交修改请求
				Debug.println("接收到修改商业IC卡卡表信息请求，取得修改的值，提交修改请求");
				CardTypeStruct cardTypeStruct=getCardDataA(request);
				...
				try{
					...
					request.setAttribute("targetURL",target);
					request.setAttribute("btnTitle",btnTitle);
					Message="成功修改商业IC卡卡表信息，但";
					throw new MonitorException(mexp.getErrCode(),Message+mexp.getMessage());
				}finally{
					//设置日志信息
					String userOperRstDesc="修改前商业IC卡卡表信息:";
				...
				if(queryList==null || queryList.size()==0){
					...
					lTarget="/ToucsMonitor/POSDeviceManage/CardQuery.jsp";
					...
					request.setAttribute("cardClass",queryCode.getCardClass());
					request.setAttribute("cardType",queryCode.getCardType());
					lTarget="/ToucsMonitor/POSDeviceManage/CardModify.jsp";
					...
					request.setAttribute("cardClass",queryCode.getCardClass());
					request.setAttribute("cardType",queryCode.getCardType());
					lTarget="/ToucsMonitor/POSDeviceManage/CardQueryRst.jsp";
				}
				...
				try{
					...
					request.setAttribute("targetURL",target);
					request.setAttribute("btnTitle",btnTitle);
					String Message="成功从数据库中删除商业IC卡卡表信息，但";
					retMessage=Message+mexp.getMessage();
				}finally{
					//设置日志信息
					String userOperRstDesc="删除一条商业IC卡卡表信息,卡表信息:";
					userOperRstDesc=userOperRstDesc+"磁道号="+cardTypeStruct.getTrack_no()+"适配偏移="+cardTypeStruct.getMatchStart();
					userOperRstDesc=userOperRstDesc+"适配标识="+cardTypeStruct.getcardMatch()+"卡表状态="+cardTypeStruct.getrec_useflag();
					userOperRstDesc=userOperRstDesc+"卡表日期时间戳="+cardTypeStruct.getrec_datetime()+"银行名称="+cardTypeStruct.getbankCode();
					userOperRstDesc=userOperRstDesc+"；返回的处理结果："+retMessage;
				...
				request.setAttribute("cardClass",queryCode.getCardClass());
				request.setAttribute("cardType",queryCode.getCardType());
				lTarget="/ToucsMonitor/POSDeviceManage/HandCardQueryRst.jsp";
				...
				if(loginfo!=null){
					...
					//取消操作即是重新查询后回到卡表列表页面
					String nurl="/ToucsMonitor/posicservlet?reqCode="+CANCEL;
	...
	private LoginInfo commProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException,MonitorException,MonitorException{
		...
		//取得用户资源权限
		Hashtable opvHashtable=POSICCardBean.getOperRes(maskCode);
	...
	private String cancel(HttpServletRequest request) throws MonitorException{
		...
		return "/ToucsMonitor/POSDeviceManage/CardQueryRst.jsp";
	}
}