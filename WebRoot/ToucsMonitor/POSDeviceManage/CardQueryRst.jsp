...
<%
	...
	if(obj1!=null)msg=(String)obj1;
  	String REGCARD="13001";
  	String MODCARD="13002";

  	String MODCARDREQ2="1300202";
  	String DELCARD="13003";
  	String QUERYCARD="13004";
  	String MG7550="12015";
...
function submit11(type,index){
    if(type==2)
		if(confirm("确认要删除选择的商业IC卡卡表信息？")!=true) return false;
	var url="";
	if(type==1){					
		url="/ToucsMonitor/posicservlet?reqCode=<%=MODCARDREQ2%>&&uid=<%=uid%>";
	}else{
		url="/ToucsMonitor/posicservlet?reqCode=<%=DELCARD%>&&uid=<%=uid%>";
	}
...
function refreshsubmit(){
	if(confirm("确认要刷新商业IC卡卡表信息吗？")!=true) return false;
	var url="/ToucsMonitor/posicservlet?reqCode=<%=MG7550%>";
	document.form1.action=url;
	document.form1.submit();		
	return true;
}
...
function SelPage(){
	var url="/ToucsMonitor/posicservlet?reqCode=<%=PAGEPROCESSOR%>";
	document.form1.action=url;
	document.form1.submit();		
}
  	...
  	<table width="98%" border="0" cellpadding="0" cellspacing="0" align="left">
    	<tr> 
        	...
        	<%
	     		String  nUrl="";
	     		if(hasQ.equals("0")){
		 			nUrl="/ToucsMonitor/posicservlet?reqCode="+QUERYCARD+"&&uid="+uid;
	    	%>
        	...
        	<%	
        		if(hasA.equals("0")){
					nUrl="/ToucsMonitor/posicservlet?reqCode="+REGCARD+"&&uid="+uid;
			%>
        	...
        	<%
        		if(hasM.equals("0")){
					nUrl="/ToucsMonitor/posicservlet?reqCode="+MODCARD+"&&uid="+uid;
			%>
  	...
  	<table width="98%" cellspacing="1" cellpadding="1" bordercolor="#000000" border="0" bgcolor="#000000">
      	...
      	<%
	  			...
	  			if(bankCodeList==null){
					...
					for(int J=0;J<bankCodeList.size();J++){
						try{
			   				POSICCompanyIdInfo  CodeBean0=(POSICCompanyIdInfo)bankCodeList.get(J);
		...
		%>
        	...
        	<%
        		if(hasM.equals("0")){
        	%>
        			<a href="/ToucsMonitor/posicservlet"  onClick="submit11(1,<%=i%>); return false;">[修改]</a> 
        	<%
        		}
        	%>
        	<%
        		if(hasD.equals("0")){
        	%>
        			<a href="/ToucsMonitor/posicservlet" onClick="submit11(2,<%=i%>); return false;">[删除]</a> 
      		</td>
      		<%
      			}
      		%>
