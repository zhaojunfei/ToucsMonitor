...
function TextIsNull(textName,maxDigit){
 ...
 function check(){
	...
	function cancel11(){
			var url="/ToucsMonitor/posicservlet?reqCode=<%=CANCEL%>&&uid=<%=uid%>";
	...
	function submit11(type){
		...
		var url;
		//if(type==1){
			//url="/ToucsMonitor/posicservlet?reqCode=<%=MODCARDREQ1%>&&uid=<%=uid%>";
		//}else if(type==2){
			//url="/ToucsMonitor/posicservlet?reqCode=<%=QUERYHANDCARD%>&&uid=<%=uid%>";
		//}
		url="/ToucsMonitor/posicservlet?reqCode=<%=reqCode%>&isFirstTime=1";
		document.form1.action=url;
		document.form1.submit();
	}
</script>
...
<form name="form1" method="post" action="/ToucsMonitor/posicservlet?reqCode=13005">
  <h2><font face="隶书">商业IC卡手输卡表查询</font> </h2>
  <hr noshade>
