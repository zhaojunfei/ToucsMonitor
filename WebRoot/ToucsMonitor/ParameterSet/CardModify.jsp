...
function check(){    
	...
	if(!confirm("确认要修改卡表信息吗?按下确定按钮后将修改卡表信息，此次修改将影响所有的ATM卡表信息！")) return false;
	document.form1.action="/ToucsMonitor/parametersetservlet?reqCode=<%=MODCARD%>&&uid=<%=uid%>";	
          		...
          		<%			
			   		...
			   		if(bankCodeList!=null){
						...
						for(int i=0;i<bankCodeList.size();i++){
							...
							if(strBankCode.equals(bankcode)){
								sel="selected";																 				  
							}
				%>
