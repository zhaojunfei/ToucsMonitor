...
<%
  	...
  	if(login != null){
    	try{
    		if( LoginManageBean.operValidate(login,"16006") == 0){
    			DisStr = "";
    		}
    	}catch(Exception ex){
    		ex.printStackTrace();
...
function SelPage(){
	...
	var url="/ToucsMonitor/parametersetservlet?reqCode=16005&page="+page+"&isFirstTime=0";
	document.form1.action=url;
	document.form1.submit();
}
function UpdateSubMit(){
    if(confirm("确实要修改？")){
        var url="/ToucsMonitor/parametersetservlet?reqCode=16006";
        document.form1.action=url;
        document.form1.submit();
        return true;
    }
    				...
    				<tr>
        				...
        				%>
        					<input type="button" value="返回" name="cmdCancel"  class="inputbutton" style="width: 120"
             					onClick=javascript:location.href="/ToucsMonitor/parametersetservlet?reqCode="+"<%= QUERYHANDCARD %>"></td>
			...
			<td>
  				<table width="745" cellspacing="1" cellpadding="1" bordercolor="#000000" border="0" bgcolor="#000000">
    				...
    				<%
						...
						if(handflag!=null){
							handflag=handflag.trim();
						}
						if( handflag.equalsIgnoreCase("1")) {
					%>
						<td nowrap > <a href="parametersetservlet?reqCode=16006&track_no=<%=cardTypeStruct.getTrack_no()%>&matchStart=<%=cardTypeStruct.getMatchStart()%>&cardMatch=<%=cardTypeStruct.getcardMatch()%>&curPage=<%=currPageNum%>&flag=0"><font color="#FF0000">设置不可手输</font></a></td>
					<%
						} else {
					%>
						<td nowrap > <a href="parametersetservlet?reqCode=16006&track_no=<%=cardTypeStruct.getTrack_no()%>&matchStart=<%=cardTypeStruct.getMatchStart()%>&cardMatch=<%=cardTypeStruct.getcardMatch()%>&curPage=<%=currPageNum%>&flag=1">设置可手输</a></td>
					<%
						}
					%>
      				<%}%>
