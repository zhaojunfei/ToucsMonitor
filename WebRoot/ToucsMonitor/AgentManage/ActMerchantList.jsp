  ...
  <div align="left">商户列表 </div>
	...
	<table width="100%" cellspacing="1" cellpadding="1" bordercolor="#000000" border="0" bgcolor="#000000">
  		...
  		<tr bgcolor="#FFFFFF" id=<%=info.getMerchantid()%>>
    		...
    		<td align="center">
        		<a href="<%=getOperLink("17503","page",merchant_id,uid)%>">[修改] </a>
        		<a href="javascript:onDelete('<%=getOperLink("17502","",merchant_id,uid)%>')">[删除]</a>
       			<%if(info.getMerchantstat().equals("0")){%>
        		<a href="<%=getOperLink("17505","",merchant_id,uid)%>">[启用]</a>
        		<%}%>
        		<%if(info.getMerchantstat().equals("1")){%>
        		<a href="<%=getOperLink("17505","page",merchant_id,uid)%>">[停用]</a>
        		<%}%>
        		<%if(info.getSecukind().equals("0")){%>
        		<a href="<%=getOperLink("17506","",merchant_id,uid)%>">[生成主密钥]</a>
        		<a href="<%=getOperLink("17506","page",merchant_id,uid)%>">[签到]</a>
        		<%}%>
    		</td>
  		</tr>
  		<%}  }%>
